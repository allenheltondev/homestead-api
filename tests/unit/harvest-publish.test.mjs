...
jest.unstable_mockModule("../../api/lib/grn.mjs", () => ({ createListing, expireListing }));
...
describe("POST /harvest-logs/:id/publish", () => {
  test("builds a listing, POSTs to GRN with harvest id as idempotency key, stores grn fields", async () => {
    getHarvestLog.mockResolvedValue({
      id: "h1", cropName: "Tomato", quantity: 5, unit: "lb", harvestedAt: "2026-06-15T00:00:00.000Z",
    });
    ...
    const event = { body: JSON.stringify({ cropId: UUID, availableEnd: "2026-06-30" }) };
  ...
  test("409 when already published", async () => {
    getHarvestLog.mockResolvedValue({ id: "h1", grnListingId: "L1" });
    const app = fakeApp();
    registerHarvestRoutes(app);
    const event = { body: JSON.stringify({ cropId: UUID, availableEnd: "2026-06-30" }) };
